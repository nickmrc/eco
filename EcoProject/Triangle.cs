...
namespace EcoProject
{
    public class Triangle: IComparable<Triangle>
    {
        ...
        public Vertex Сentroid
        {
            set 
            {
                float sumX = M1.x + M2.x + M3.x;
                float sumY = M1.y + M2.y + M3.y;
        ...
        public string sE1
        {
           get { return this.E1.ToString(); }
        }

        public string sE2
        {
            get { return this.E2.ToString(); }
        }

        public string sE3
        {
            get { return this.E3.ToString(); }
        }
        ...
        public float V4;
        public float DensityOfTriangle { get; set; } = new float();
        ...
        {
            V4 = new float();
            DensityOfTriangle = new float();
            foreach (Edge item in edgemas)
            {
               item.Update(); 
            }
            this.CALCED = false;
        ...
        {
            return Math.Max(Math.Max(this.M1.x, this.M2.x), this.M3.x);
        }

        int IComparable<Triangle>.CompareTo(Triangle other)
        {
            float sumOther = other.MaxSide();
            float sumThis = this.MaxSide();
